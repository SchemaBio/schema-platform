...
import { KNOWLEDGE_TAB_CONFIGS } from '../types';
...
export function KnowledgeTabs({ activeTab, onTabChange, children }: KnowledgeTabsProps) {
  const tabRefs = React.useRef<Map<KnowledgeTabType, HTMLButtonElement>>(new Map());

  // 键盘导航处理
  const handleKeyDown = React.useCallback((e: React.KeyboardEvent, currentIndex: number) => {
    const tabCount = KNOWLEDGE_TAB_CONFIGS.length;
    ...
    if (newIndex !== null) {
      const newTab = KNOWLEDGE_TAB_CONFIGS[newIndex];
    ...
    <div>
      ...
      <div className="border-b border-border-default mb-4">
        <nav
          ...
          role="tablist"
          aria-label="历史检出位点标签页"
        >
          {KNOWLEDGE_TAB_CONFIGS.map((tab, index) => {
